...
// ─── demo helper routes (No Auth required for presentation convenience) ──
app.post("/api/demo/reset", async (c) => {
  ...
  // Create default mock data so it's not empty on first demo
  const list = [
...
app.post("/api/demo/simulate", async (c) => {
  const body = await c.req.json().catch(() => null);
